#!/usr/bin/env python3
"""Freeze formal_v3 fixed dev-4 no_grad step0 baseline."""
...
OUT = GRPO_DIR / "grpo_formal_v3_fixed_dev4_step0_baseline.json"
...
def main() -> None:
    ...
    authorization = json.loads(V2_AUTH.read_text(encoding="utf-8"))
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA unavailable for fixed dev-4 baseline")
    ...
    baseline = {
        "format_version": 1,
        "status": "frozen_before_formal_v3_training_no_optimizer_step",
        "run_id": "grpo-formal-v3",
        "source": "fixed_dev4_no_grad_probe_step0",
        "prompt_ids": probe["probe_prompt_ids"],
        "prompt_id_set_is_fixed_dev4": probe[
        ...
        "factual_precision_mean_by_intent": (
            probe["factual_precision_mean_by_intent"]
        ),
        ...
        "frozen_hashes": {
            "formal_v2_authorization_sha256": sha256_file(V2_AUTH),
            "signal_probe_input_manifest_sha256": sha256_file(INPUT_PATH),
    ...
    print(
        json.dumps(
            {
                "baseline_path": str(OUT.relative_to(ROOT)),
